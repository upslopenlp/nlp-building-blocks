 ...
 * the License.
 ******************************************************************************/
package com.mtnfog.sonnet.models.generate;
...
public abstract class BaseStandardModelGenerator extends BaseRunnable {
	...
	public boolean validateTrainingDefinition(TrainingDefinitionReader reader) {
		...
		// Make sure the encryption key is not empty.
		if(StringUtils.isEmpty(reader.getTrainingDefinition().getModel().getEncryptionkey())) {
			output("The model encryption key cannot be empty.");
			return false;
		}
	 ...
	 * value will be formatted as <code>sonnet-1.1.0</code> where 1.1.0 is the
	 * version of Sonnet that generated the model.
	 */
	public String getCreatorVersion() {
		
		return "sonnet-" + getVersion();
	...
	public void generateManifest(File manifestFile, String modelId, String modelFile, TrainingDefinitionReader reader) throws TrainingDefinitionException, IOException {
		...
		ModelManifestUtils.generateStandardModelManifest(manifestFile, modelId, name, type, StandardModelManifest.DEFAULT_SUBTYPE, modelFile, languageCode, encryptionKey, getCreatorVersion(), source);
